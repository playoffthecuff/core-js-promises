...
function getAllResult(promises) {
  return Promise.allSettled(promises).then(function resolve(res) {
    return res.map((item) => (item.status === 'rejected' ? null : item.value));
  });
}
...
function queuPromises(promises) {
  let startStr = Promise.resolve('');
  promises.forEach((promise) => {
    startStr = startStr.then((results) => {
      return promise.then((result) => {
        const concatStr = results + result;
        return concatStr;
      });
    });
  });
